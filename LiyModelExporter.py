...
if(exportVertexColors == 1):
    ...
    for f in bm.faces:
        for loop in f.loops:        
            lala1 = str(int(min(255, math.sqrt(loop[color].x) * 255)))
            lala2 = str(int(min(255, math.sqrt(loop[color].y) * 255)))
            lala3 = str(int(min(255, math.sqrt(loop[color].z) * 255)))
            out1 = lala1 + " " + lala2 + " " + lala3 + " "
            file.write(out1)
